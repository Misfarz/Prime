...
const saveProduct = async (req, res) => {
    try {
        ...
        const existingImageKeys = Object.keys(req.body).filter(key => key.startsWith('existingImages_'));
        ...
        if (existingImages && finalImages.length === 0) {
            if (Array.isArray(existingImages)) {
                const processedExistingImages = existingImages.map(img => img.split('?')[0]);
