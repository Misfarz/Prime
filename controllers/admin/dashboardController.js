...
const downloadExcelReport = async (req, res) => {
  try {
    ...
    worksheet.getCell("A2").value = `Period: ${formatDate(
      startDate
    ...
    worksheet.getCell("A3").value = `Total Orders: ${
      orders.length
...
const downloadPdfReport = async (req, res) => {
  try {
    ...
    doc
      .fontSize(12)
      .text(
        `Total Orders: ${orders.length} | Total Sales: ₹${totalSales.toFixed(
          2
        )} | Total Discount: ₹${totalDiscount.toFixed(2)}`,
        { align: "center" }
      );
  ...
  try {
    ...
    if (filterType === "yearly") {
      ...
      while (currentMonthIter <= endMonthIter) {
        const monthKey =
          monthNames[currentMonthIter.getMonth()] +
          " " +
          currentMonthIter.getFullYear();
        salesByMonth[monthKey] = 0;
        currentMonthIter.setMonth(currentMonthIter.getMonth() + 1);
      }
      ordersForTrendChart.forEach((order) => {
        const monthKey =
          monthNames[order.createdAt.getMonth()] +
          " " +
      ...
      if (filterType === "monthly")
        salesChartTitle = `Sales Trend Over the Last 30 Days (Daily)`;
      else if (filterType === "weekly")
        salesChartTitle = `Sales Trend Over the Last 7 Days (Daily)`;
      else if (filterType === "daily") salesChartTitle = `Sales Trend Today`;
      else if (filterType === "custom")
        salesChartTitle = `Sales Trend: ${formatDate(
          startDate
        )} to ${formatDate(endDate)} (Daily)`;
      ...
      ordersForTrendChart.forEach((order) => {
        ...
        if (salesByDay.hasOwnProperty(dayString))
          salesByDay[dayString] += order.total;
      });
    ...
    if (periodStats.totalSales > 0) {
      averageSalesValue =
        periodStats.totalSales / (diffDays > 0 ? diffDays : 1);
    }
